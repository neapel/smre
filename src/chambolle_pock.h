 ...
 * 		w_i &= K_i^* * y^{n + 1}_i \\
 * 		w &= \sum_i w_i \\
 * 		x^{n + 1} &= x^n - \tau_n w + \tau_n y / (1 + \tau_n) \\
 * 		\theta_n &= 1 / \sqrt{1 + \lVert 2 \tau_n y \rVert} \\
 * 		\tau_{n + 1} &= \theta_n \tau_n \\
 * 		\sigma_{n + 1} &= \sigma_n / \theta_n \\
 * 		\bar x^{n + 1} &= x^{n + 1} + \theta_n (x^{n + 1} - x^n)
 * 	\f}
 */
boost::multi_array<float, 2> chambolle_pock(float tau, float sigma, boost::multi_array<float, 2> &x, std::vector<constraint>);
